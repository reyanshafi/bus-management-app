...
exports.updateBusLocation = onRequest(async (req, res) => {
  ...
  try {
    ...
    logger.info('Received data:', gpsData);
    
    // Check if this is a status update
    if (gpsData.isStatusUpdate === true) {
      // Handle status update (no GPS coordinates required)
      if (!gpsData.busId || !gpsData.status || !gpsData.message) {
        res.status(400).json({ 
          error: 'Status update missing required fields: busId, status, message' 
        });
        return;
      }

      // Update bus document with status information
      ...
      if (busDoc.exists) {
        await busRef.update({
          ...
          message: gpsData.message,
          lastStatusUpdate: admin.firestore.FieldValue.serverTimestamp(),
          isActive: true
        });
        logger.info(`Status updated for bus ${gpsData.busId}: ${gpsData.status}`);
      } else {
        logger.warn(`Bus document ${gpsData.busId} does not exist for status update`);
    ...
    // Handle GPS location update (requires coordinates)
    if (!gpsData.busId || gpsData.latitude === undefined || gpsData.longitude === undefined) {
      res.status(400).json({ 
        error: 'GPS update missing required fields: busId, latitude, longitude' 
      });
      return;
    }
